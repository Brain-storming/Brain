...
public class XlxfGame extends BaseActivity {

	TextView textView[] = new TextView[36];
	String returnValue[] = new String[36];
	Button yonghu1Button, yonghu2Button;
	String touchChengyu = "";
	Handler handler = null;
	int flag = 0;
	int[] record = new int[4];
	int mProgressStatus1, mProgressStatus2 = 0;// 完成进度起始值
	ProgressBar yonghu1Bar, yonghu2Bar;
	Handler jishi1Handler, jishi2Handler;

	@Override
	protected void onCreate(Bundle savedInstanceState) {
		super.onCreate(savedInstanceState);
		// 添加布局
		setContentView(R.layout.xlxf_game);

		handler = new Handler();
		jishi1Handler = new Handler();
		jishi2Handler = new Handler();
		...
		yonghu1Button = (Button) findViewById(R.id.xlxf_yonghu1);
		yonghu2Button = (Button) findViewById(R.id.xlxf_yonghu2);
		yonghu1Button.setText(Constant.userName);

		yonghu1Bar = (ProgressBar) findViewById(R.id.xlxf_jishi1);
		yonghu2Bar = (ProgressBar) findViewById(R.id.xlxf_jishi2);
				...
				if ((textView[num].getText() != "") && !zaibuzai2(record, num)) {
					...
					if (flag == 4) {
						...
						if (Chengyu.check(touchChengyu)) {
							if (Constant.soundSt == true) {
								SoundPlayer.playsound(R.raw.xiaochu_music);
							}
							System.out.println("输入成语正确！");
							// 消去相关textview中的汉字
							jishi1Handler.post(runnableJishiUi1);
							jishi2Handler.post(runnableJishiUi2);
	...
	// 为两个用户的消除进度建立线程
	Runnable runnableJishiUi1 = new Runnable() {
		@Override
		public void run() {
			// 更新界面
			mProgressStatus1 += 11;// 完成进度起始值
			yonghu1Bar.setProgress(mProgressStatus1);
		}

	};
	Runnable runnableJishiUi2 = new Runnable() {
		@Override
		public void run() {
			// 更新界面
			mProgressStatus2 += 11;// 完成进度起始值
			yonghu2Bar.setProgress(mProgressStatus2);
		}
